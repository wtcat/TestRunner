 ...
 * Platform adapt
 */
typedef struct list_head rtems_chain_control;
typedef struct list_head rtems_chain_node;
typedef pthread_cond_t rtems_condition_variable;
typedef pthread_mutex_t rtems_mutex;
...
#define rtems_chain_append_unprotected(head, link) list_add_tail(link, head)
#define rtems_chain_is_empty(head) list_empty(head)
#define rtems_chain_set_off_chain(link) (link)->next = NULL
#define rtems_chain_is_node_off_chain(link) ((link)->next == NULL)
#define rtems_chain_insert_unprotected(after_node, the_node) \
  __list_add((the_node), (after_node), (after_node)->next)

// #define rtems_chain_get_unprotected(head) (head)->next //TODO: 
  
#define rtems_mutex_lock(_l) pthread_mutex_lock(_l)
#define rtems_mutex_unlock(_l) pthread_mutex_unlock(_l)
#define rtems_condition_variable_wait(cond_var, lock) \
  pthread_cond_wait(cond_var, lock)
#define rtems_condition_variable_broadcast(cond_var) \
  pthread_cond_broadcast(cond_var)

static inline struct list_head *rtems_chain_get_unprotected(struct list_head *head) {
  if (!list_empty(head)) {
...
typedef struct rtems_bdbuf_swapout_worker
{
  rtems_chain_node             link;     /**< The threads sit on a chain when
                                          * idle. */
  sem_t                        sem;
  pthread_t                    id;       /**< The id of the task so we can wake
                                          * it. */
...
typedef struct rtems_bdbuf_cache {
  sem_t               swapout_sync;
  pthread_t           swapout;           /**< Swapout task ID */
  ...
  sem_t              *sync_requester;    /**< The sync requester. */
  ...
  pthread_t           read_ahead_task;   /**< Read-ahead task */
  sem_t               read_ahead_sem;
...
static void *rtems_bdbuf_swapout_task(void * arg);
static void *rtems_bdbuf_read_ahead_task(void * arg);
...
static rtems_bdbuf_cache bdbuf_cache = {
  ...
  .access_waiters = { .cond_var = PTHREAD_COND_INITIALIZER },
  .transfer_waiters = {
    .cond_var = PTHREAD_COND_INITIALIZER
  },
  .buffer_waiters = { .cond_var = PTHREAD_COND_INITIALIZER },
...
{
  assert(error == 0);// rtems_fatal (RTEMS_FATAL_SOURCE_BDBUF, error);
}
...
rtems_bdbuf_wake_swapper (void)
{
  ...
  if (sem_post(&bdbuf_cache.swapout_sync) != RTEMS_SUCCESSFUL)
...
rtems_bdbuf_get_buffer_from_lru_list (rtems_disk_device *dd,
                                      rtems_blkdev_bnum  block)
{
  rtems_chain_node *node, *n;

  list_for_each_safe(node, n, &bdbuf_cache.lru) {
...
)
{
    ...
    return pthread_create(id, &attr, thread_entry, arg);
}
...
static void
rtems_bdbuf_swapout_transfer_init (rtems_bdbuf_swapout_transfer* transfer, sem_t *sync)
{
  ...
  transfer->write_req.io_task = sync;
...
static void *
rtems_bdbuf_swapout_worker_task (void * arg);
...
rtems_bdbuf_swapout_workers_create (void)
{
  ...
  sc = worker_current != NULL ? RTEMS_SUCCESSFUL : -ENOMEM;
       ...
       w++, worker_current += worker_size)
  {
    ...
    rtems_bdbuf_swapout_transfer_init (&worker->transfer, &worker->sem);
    rtems_chain_append_unprotected (&bdbuf_cache.swapout_free_workers, &worker->link);
    worker->enabled = true;
    sc = rtems_bdbuf_create_task ("bdbuf-worker",
    ...
    if (sc != RTEMS_SUCCESSFUL)
    {
      ...
      rtems_chain_extract_unprotected (&worker->link);
...
rtems_bdbuf_do_init (void)
{
  ...
  if (rtems_bdbuf_read_request_size (bdbuf_config.max_read_ahead_blocks)
      > RTEMS_BDBUF_TASK_STACK_SIZE_DEFAULT / 8U)
    return -EINVAL;
   ...
   */
  bdbuf_cache.bds = general_calloc (sizeof (rtems_bdbuf_buffer),
                            bdbuf_cache.buffer_min_count);
  if (!bdbuf_cache.bds)
    goto error;
   ...
   * Allocate the memory for the buffer descriptors.
   */
  bdbuf_cache.groups = general_calloc (sizeof (rtems_bdbuf_group),
                               bdbuf_cache.group_count);
   ...
   */
  bdbuf_cache.buffers = general_aligned_alloc(bdbuf_config.cpu_cacheline_size, 
  ...
  sem_init(&bdbuf_cache.swapout_sync, 0, 0);
  rtems_bdbuf_swapout_transfer_init (bdbuf_cache.swapout_transfer, 
    &bdbuf_cache.swapout_sync);
  sc = rtems_bdbuf_create_task ("bdbuf-swap",
  ...
  {
    sem_init(&bdbuf_cache.read_ahead_sem, 0, 0);
    bdbuf_cache.read_ahead_enabled = true;
    sc = rtems_bdbuf_create_task ("read-ahead",
...
error:
  ...
  if (bdbuf_cache.read_ahead_task != 0)
    pthread_cancel(bdbuf_cache.read_ahead_task);

  if (bdbuf_cache.swapout != 0)
    pthread_cancel(bdbuf_cache.swapout);
                                      ...
                                      bool                  cache_locked)
{
  ...
  if (sc == RTEMS_SUCCESSFUL || sc == -ENOSYS)
    return sc;
  else
    return -EIO;
}
...
rtems_bdbuf_read_ahead_add_to_chain (rtems_disk_device *dd)
{
  ...
  {
    if (sem_post(&bdbuf_cache.read_ahead_sem) != RTEMS_SUCCESSFUL)
...
rtems_bdbuf_syncdev (rtems_disk_device *dd)
{
  ...
  bdbuf_cache.sync_requester = &sync_sem;
  // bdbuf_cache.sync_requester = rtems_task_self ();
  bdbuf_cache.sync_device    = dd;

  rtems_bdbuf_wake_swapper ();
  rtems_bdbuf_unlock_cache ();
  if (sem_wait(&sync_sem))
    rtems_bdbuf_fatal (RTEMS_BDBUF_FATAL_WAIT_TRANS_EVNT);
  // rtems_bdbuf_wait_for_transient_event ();
  rtems_bdbuf_unlock_sync ();
  ...
  if (!rtems_chain_is_empty (&transfer->bds))
  {
    ...
    while ((node = rtems_chain_get_unprotected(&transfer->bds)) != NULL)
    {
      ...
      if (write)
      {
        ...
        transfer->write_req.status = -EBUSY; //RTEMS_RESOURCE_IN_USE;
        transfer->write_req.bufnum = 0;
      }
    }
                                         ...
                                         uint32_t             timer_delta)
{
  if (!rtems_chain_is_empty (chain))
  {
    rtems_chain_node* node = chain; //rtems_chain_head (chain);
    bool              sync_all;
    ...
    while (!(chain == node) /*!rtems_chain_is_tail (chain, node)*/)
    {
      rtems_bdbuf_buffer* bd = (rtems_bdbuf_buffer*) node;
      ...
      if (bd->dd == *dd_ptr)
      {
        ...
        rtems_chain_node* tnode = transfer; //rtems_chain_tail (transfer);
        ...
        tnode = tnode->prev; //tnode = tnode->previous;

        while (node && !(transfer == tnode) /*!rtems_chain_is_head (transfer, tnode)*/)
        {
          ...
          if (bd->block > tbd->block)
          {
            ...
            tnode = tnode->prev; //tnode->previous;
        }
                                ...
                                rtems_bdbuf_swapout_transfer* transfer)
{
  ...
  if (!rtems_chain_is_empty (&transfer->bds))
  {
    if (worker)
    {
      if (sem_post(&worker->sem) != RTEMS_SUCCESSFUL)
  ...
  {
    sem_t *sync_requester; //rtems_id sync_requester;
    rtems_bdbuf_lock_cache ();
    ...
    rtems_bdbuf_unlock_cache ();
    if (sync_requester)
      sem_post(sync_requester); //rtems_event_transient_send (sync_requester);
  }
...
rtems_bdbuf_swapout_workers_close (void)
{
  ...
  list_for_each(node, &bdbuf_cache.swapout_free_workers) {
 ...
 */
static void *
rtems_bdbuf_swapout_task (void * arg)
{
...
static void *
rtems_bdbuf_read_ahead_task (void * arg)
{
  rtems_chain_control *chain = &bdbuf_cache.read_ahead_chain;
  ...
  while (bdbuf_cache.read_ahead_enabled)
  {
    ...
    if (sem_wait(&bdbuf_cache.read_ahead_sem))
      rtems_bdbuf_fatal (RTEMS_BDBUF_FATAL_WAIT_EVNT);
