...
#include "basework/lib/blkdev.h"
#include "basework/lib/diskdev.h"
...
{
  struct list_head link;       /**< Link the BD onto a number of lists. */
...
struct rtems_bdbuf_group
{
  struct list_head    link;          /**< Link the groups on a LRU list if they
...
typedef struct rtems_bdbuf_config {
  ...
  uint32_t            max_write_blocks;        /**< Number of blocks to write
                                                * at once. */
  int swapout_priority;        /**< Priority of the swap out
                                                * task. */
  ...
  size_t              swapout_workers;         /**< The number of worker
                                                ...
                                                * task. */
  int swapout_worker_priority; /**< Priority of the swap out
                                                * task. */
  ...
  uint32_t            buffer_max;              /**< Maximum buffer size
                                                ...
                                                * allocation size. */
  int read_ahead_priority;     /**< Priority of the read-ahead
                                                * task. */
  size_t              cpu_cacheline_size;
} rtems_bdbuf_config;
 ...
 */
#define RTEMS_BDBUF_TASK_STACK_SIZE_DEFAULT 1024
